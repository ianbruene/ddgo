...
- Connection establishment is an explicit controller admission transition reserved before the potentially blocking transport open. Response owners and realtime/manual operations cannot be admitted while connecting, only one `Connect` can run at a time, and commands become eligible only after both transport open and connected-state commit have completed. Failed or canceled opens release the reservation. Connect and disconnect use mutually exclusive transitions, so they cannot overlap; disconnect during connect is rejected and connect cancellation is performed through the connect context.
